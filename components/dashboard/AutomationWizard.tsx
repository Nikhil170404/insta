...
export default function AutomationWizard({ selectedMedia, onClose, onSave, saving }: AutomationWizardProps) {
    const [step, setStep] = useState(1);

    // State for different steps
    const [triggerType, setTriggerType] = useState<"specific" | "any" | "next">("specific");
    const [matchingType, setMatchingType] = useState<"any" | "keyword">("keyword");
    const [keywords, setKeywords] = useState("");
    const [replyToComments, setReplyToComments] = useState(false);

    const [openingDM, setOpeningDM] = useState("Hey there! I'm so happy you're here, thanks so much for your interest 😊\n\nClick below and I'll send you the link in just a sec ✨");
    ...
    const [respondToReplies, setRespondToReplies] = useState(false);
    const [ignoreSelfComments, setIgnoreSelfComments] = useState(true);
    ...
    const handleSave = () => {
        onSave({
            trigger_type: matchingType === "any" ? "any" : "keyword",
            trigger_keyword: keywords,
            reply_message: openingDM,
            comment_reply: replyToComments ? "Check your DMs! 📬" : null,
            button_text: buttonText,
            link_url: linkUrl,
            require_follow: requireFollow,
            respond_to_replies: respondToReplies,
            ignore_self_comments: ignoreSelfComments,
            // We might want to expand this to support the "Final DM" separately later
            // For now, we'll keep it compatible with our existing schema
        });
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4 lg:p-10">
            ...
            {/* Wizard Container */}
            <div className="relative w-full h-full md:h-auto md:max-w-xl bg-white md:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-xs text-slate-400 font-medium tracking-wide flex items-center gap-1.5 uppercase">
                            Step {step} of 4 <span className="w-1 h-1 rounded-full bg-slate-300" /> {step === 1 ? "Trigger" : step === 2 ? "Matching" : step === 3 ? "Message" : "Final Link"}
                        </p>
                    </div>
                    <button onClick={onClose} className="p-2.5 bg-slate-50 hover:bg-slate-100 text-slate-500 rounded-2xl transition-all">
                        <X className="h-5 w-5" />
                    </button>
                </div>
                ...
                {/* Footer */}
                <div className="px-6 py-5 border-t border-slate-50 bg-slate-50/30 flex items-center gap-3">
                    {step > 1 && (
                        <button
                            onClick={prevStep}
                            className="px-6 py-4 bg-white border border-slate-100 text-slate-600 font-bold rounded-2xl active:scale-95 transition-all flex items-center gap-2"
                        >
                    ...
                    <Button
                        ...
                        disabled={saving}
                        className="flex-1 h-14 text-sm font-bold rounded-2xl bg-primary hover:bg-primary/90 text-white shadow-xl shadow-primary/20 active:scale-95 transition-all"
                    >
                        {saving ? (
