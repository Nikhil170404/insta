...
export default function WaitlistClient({ alreadyJoined }: WaitlistClientProps) {
    const [stats, setStats] = useState<WaitlistStats | null>(null);
    const [username, setUsername] = useState("");
    const [whatsapp, setWhatsapp] = useState("");
    const [loading, setLoading] = useState(false);

    // Initial success state if user already joined
    const [success, setSuccess] = useState<{
        position: number;
        tier: string;
        message: string;
    } | null>(alreadyJoined ? {
        position: 0, // Indicates unknown position (returned user)
        tier: "unknown",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                // If user already joined (409), treat as success essentially but with error message logic?
                // Or just show error. The API returns 409 for duplicate.
    ...
    const getTierIcon = (tier: string) => {
        switch (tier) {
            ...
            default: return <Gift className="h-6 w-6 text-emerald-500" />; // Fallback for discount/unknown
        }
    };
                ...
                <div className="max-w-lg mx-auto mb-12 relative z-10">
                    {success ? (
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 p-8 md:p-10 text-center animate-in fade-in zoom-in-95 duration-500">
                            ...
                            {/* Fallback Icon for Returning Users */}
                            {success.tier === "unknown" && (
                                <div className="w-20 h-20 rounded-[1.5rem] mx-auto mb-6 flex items-center justify-center bg-gradient-to-br from-emerald-500 to-teal-500 shadow-xl shadow-emerald-500/20">
                                    <Check className="h-10 w-10 text-white" />
                                </div>
                            )}

                            <h2 className="text-2xl md:text-3xl font-[900] text-slate-900 tracking-tight mb-2">
                                {success.tier === "unknown" ? "Welcome Back! 👋" : "You're In! 🎉"}
                            </h2>
                        ...
                        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 p-8 md:p-10 text-center animate-in fade-in zoom-in-95 duration-500">
                            ...
                            <div className="bg-slate-50 rounded-2xl p-6 mb-8 border border-slate-100">
                                ...
                                <p className="text-lg text-slate-300 font-bold uppercase tracking-widest">
                                    to unlock the offer
                                </p>
                            </div>
